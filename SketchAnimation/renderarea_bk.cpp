	...
	: QGLWidget(QGLFormat(QGL::AlphaChannel|QGL::SampleBuffers), parent)
{
	...
	image = QImage(IMAGE_SIZE,IMAGE_SIZE,QImage::Format_ARGB32);	//32bit color
	...
	if(SKETCH == behaviorMode)
	{
		if(event->button() == Qt::LeftButton)
		{
			...
			else // The user is sketching animation
			{
				...
				// show the fit curve
				m_vPoints.assign(resamplePoints.begin(),resamplePoints.end());
				stylePen.setColor(QColor(0,255,0));
				paint(styleUsrDrawingImage,stylePen);
				...
				m_TrajectoryFeature.push_back(feature);
			}
		}
	}
	else if(VIEW == behaviorMode)
...
void RenderArea::initializeGL()
{
	...
	// test function 13-4-10
	m_pMotion = new Motion(".\\Resources\\02_01.amc",0.6,m_pSkeleton);

	vector<Posture> posture_arr;
	for(int i = 0; i < m_pMotion->m_NumFrames; i++)
	{
		posture_arr.push_back(*m_pMotion->GetPosture(i));
	}

	emit sendNewMotion(posture_arr);
}
...
void RenderArea::loadMocapData(const char* filename,const char* type, int* p_start, int* p_end)
{
	...
	{
		qDebug("loading mocap data file %s",filename);
		Motion* m = new Motion(filename,0.6,m_pSkeleton);
