...
void Matrix::add(const Matrix& other, double scaleA, double scaleB){
	...
	{	
		//gsl_matrix_add_scaled(this->matrix, other.matrix, scaleA, scaleB); // the version 1.13 of gsl has not supported this funciton modified by lvp 11-2-25
...
// by Huansen
// get the index-th column of the matrix
Vector3d Matrix::getColumn(int index) {
	Vector3d tmp(
		MATRIX_AT(this->matrix, 0, index),
		MATRIX_AT(this->matrix, 1, index),
		MATRIX_AT(this->matrix, 2, index)
		);
	return tmp;
}

/************************************************************************/
/* Find the min and max value in the matrix  added by lvp 11-4-30       */
/************************************************************************/
void Matrix::findMinMax(double* min_out, double* max_out)
{
	gsl_matrix_minmax (this->matrix, min_out, max_out);
}

/***************************************************************************/
/* Find the index of min and max value in the matrix  added by lvp 11-4-30 */
/***************************************************************************/
void Matrix::findMinMax(int imin_out, int jmin_out, int imax_out, int jmax_out)
{
	gsl_matrix_minmax_index (this->matrix, (size_t*)&imin_out, (size_t*)&jmin_out, (size_t*)&imax_out, (size_t*)&jmax_out);
}
}