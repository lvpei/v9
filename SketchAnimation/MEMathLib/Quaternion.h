...
public:
	...
	*/
	...
	/**
		This method returns a quaternion that was the result of subtraction the quaternion rhs to the current quaternion. added by lvp 11-4-17
	*/
	Quaternion operator - (const Quaternion &rhs) const;

	/**
		This method subtractions the rhs quaternion to the current one. added by lvp 11-4-17
	*/
	Quaternion& operator -= (const Quaternion &rhs);
...
//	double decomposeRotation(const Vector3d vB, double *rotAngle, Vector3d* rotAxis) const;
	...
	*/
	...
	//add by Huansen//////////////////////////////////////////////////////////////////////////
	void normalize();
	void fromAxisAngle(const double angle, const double x, const double y, const double z);
	void fromEulerAnglesXYZ(const double angle[3]);
	void fromEulerAnglesXZY(const double angle[3]);
	void fromEulerAnglesYXZ(const double angle[3]);
	void fromEulerAnglesYZX(const double angle[3]);
	void fromEulerAnglesZXY(const double angle[3]);
	void fromEulerAnglesZYX(const double angle[3]);
	
	void getRotationMatrix(double* M) const;

	Vector3d Logmap();

	const void Expmap(Vector3d exp);
	
	//end

	Vector3d getEulerAngle();
	...
	//     |3  7  11 15|
	//      ----------- 
	void fromTransformationMatrix(const Matrix& transmat);
	void fromTransformationMatrix(const TransformationMatrix& transmat);
	void fromTransformationMatrix(const double* M);
	// end
