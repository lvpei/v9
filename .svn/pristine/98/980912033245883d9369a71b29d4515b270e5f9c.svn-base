...
Image* BMPIO::loadFromFile(int imageModel){
	...
	if(!fp){
		//throwError("cannot open the bmp file.");
		return NULL;
	}
	BMPHeader *header = new BMPHeader(fp);
	BMPInfoHeader *infoHeader = new BMPInfoHeader(fp);

	AUX_RGBImageRec *pAuxImage=NULL;

	pAuxImage=auxDIBImageLoad(fileName);
	
	if(pAuxImage==NULL)return NULL;

	Image *pNewImage=new Image(infoHeader->getNrBits()/8,pAuxImage->sizeX,pAuxImage->sizeY,pAuxImage->data);
