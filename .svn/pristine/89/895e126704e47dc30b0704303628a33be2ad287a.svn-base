...
void TransformationMatrix::setToRotationMatrixFromVectors(Vector3d from,Vector3d to){
	
	double angle=acos(from.dotProductWith(to));
	Vector3d rotateAxis=from.crossProductWith(to).toUnit();
	setToRotationMatrix(angle,rotateAxis);
}

void TransformationMatrix::setTransformationMatrixFromPoints(const Point3d &from1,const Point3d &from2,const Point3d &to1,const Point3d &to2){
	Vector3d from = from2-from1;
	Vector3d to = to2-to1;
	from.toUnit();
	to.toUnit();
	setToRotationMatrixFromVectors(from,to);
	//Point3d temp=(this->operator*(from1)+this->operator*(from2))*0.5;
	Point3d temp = this->operator *(from2);
	
	setTranslation(to2-temp);
}	
...
void TransformationMatrix::postMultiplyVector(const Vector3d& v, Vector3d& result)
{
	if (this->matrix->size1 != 4 || this->matrix->size2 != 4)
		throwError("Can only use the * operator between a 4x4 matrix and a vector3d object!");
	result.x = MATRIX_AT(this->matrix, 0, 0) * v.x + MATRIX_AT(this->matrix, 0, 1) * v.y + MATRIX_AT(this->matrix, 0, 2) * v.z + MATRIX_AT(this->matrix,0,3);
	result.y = MATRIX_AT(this->matrix, 1, 0) * v.x + MATRIX_AT(this->matrix, 1, 1) * v.y + MATRIX_AT(this->matrix, 1, 2) * v.z + MATRIX_AT(this->matrix,1,3);
	result.z = MATRIX_AT(this->matrix, 2, 0) * v.x + MATRIX_AT(this->matrix, 2, 1) * v.y + MATRIX_AT(this->matrix, 2, 2) * v.z + MATRIX_AT(this->matrix,2,3);
}
